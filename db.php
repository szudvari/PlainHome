...
function startNewYear($lastyear) {
    ...
    for ($i = 0; $i < $balrows; $i++) {
        $deposit_id = $balances[$i]['deposit_id'];
        $opening_balance = $balances[$i]['opening_balance'];
        $payments = getAllPayment($deposit_id, $lastyear);
        $ccosts = getAllCcost($deposit_id, $lastyear);
        $allocosts = getAllOcosts($deposit_id, $lastyear);
        $ocosts = $allocosts['ocost'];
        $current_balance = ($opening_balance - ($ccosts + $ocosts)) + $payments;
        $sql = "INSERT INTO `deposit_balance` (`id`, `deposit_id`, `year`, `opening_balance`, `actual_balance`) "
                . "VALUES (NULL, $deposit_id, $newyear, $current_balance, $current_balance);";
        $result = mysql_query($sql);
        if (!$result) {
            die("allDepositBalance hiba:" . mysql_errno() . " - " . mysql_error());
        }
    }
    echo "Az új évi adtok létrehozása sikeresen megtörtént.";
}
...
function getAllPayment($id, $year) {
    ...
    $result3 = mysql_query($sql);
    if (!$result3) {
        echo "GetAllPayment hiba " . mysql_errno() . ": " . mysql_error();
        exit;
    }
    while ($row = mysql_fetch_assoc($result3)) {
        $payment = $row;
...
function updateAllBalance($year) {
    ...
    for ($i = 1; $i < $depo + 1; $i++) {
        ...
        $sql = "SELECT SUM(`ccost`) as ccost FROM `ccost` WHERE `deposit_id`=$i AND `year`=$year;";
        $result = mysql_query($sql);
        if (!$result) {
...
//echo $sql;
    $result = mysql_query($sql);
    if (!$result) {
        die("depositsToDropdown hiba:" . mysql_errno() . " - " . mysql_error());
    }
    $deposits = array();
    while ($row = mysql_fetch_assoc($result)) {
        $deposits[] = $row;
...
function getAllOcosts($id, $year) {
    $sql = "SELECT `ocost`.`year`,`ocost`.`month`,`ocost`.`day`,`ocost`.`ocost`, `ocost`.`title` FROM ocost "
            . "WHERE `ocost`.`deposit_id` = $id AND `ocost`.`year` = $year "
            . "ORDER BY `ocost`.`year` DESC, `ocost`.`month` DESC, `ocost`.`day` DESC";
    $result = mysql_query($sql);
    if (!$result) {
        die("geDepositOcost hiba:" . mysql_errno() . " - " . mysql_error());
    }
    while ($row = mysql_fetch_assoc($result)) {
        $ocost[] = $row;
    }
